...
def current_price():
    q = 0
    for i, node in enumerate(nodes):
        q += node * sorted_items[i].cost
    return q

def total_efficiency():
    return current_quantity() / current_price()
...
if recursive(0):
    ...
    print("This will come to a total of ${0}, with an efficiency of {1} nuggets per dollar").format(current_price(), total_efficiency())
else:
